...
<script>
    ...
    function getSelectionsIds(){
    	...
    	ids = ids.join(",");	  //将数组转化为字符串 1,2,3,4,5
    	return ids;
    }
    ...
    var toolbar = [{
        ...
        //点击按钮的动作
        handler:function(){
            //类选择器   contains内容选择器 文本  .click()用户手动点击
        ...
        handler:function(){
        	...
        	$("#itemEditWindow").window({
        		onLoad :function(){
        			//根据list列表中的选中数据获取信息,之后修改页面数据回显.
        			var data = $("#itemList").datagrid("getSelections")[0];
        			//页面价格 price =198/100
        			data.priceView = KindEditorUtil.formatPrice(data.price);
        			//将数据信息在指定的位置进行回显  load 可以根据指定的data属性,
        			//根据key自动的实现数据的回显
        			$("#itemeEditForm").form("load",data);
        			
        			// 加载商品描述
        			$.getJSON('/item/query/item/desc/'+data.id,function(sysResult){
        				if(sysResult.status == 200){
							var itemDesc = sysResult.data;
					...
					/*	
						考点:jQuery用法
						根据cid 动态查询数据,实现数据回显  利用F12开发者工具
						选择标签,进行回显
					*/
